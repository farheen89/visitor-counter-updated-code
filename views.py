...
from .forms import TracksheetForm, DutyEntryForm, TracksheetForm1, GrievanceForm, UploadPictureForm, \
    ImageShowForm  # ,RatingForm
...
def downloadzone(request, year, month, day, year1, month1, day1, zone_name):
    ...
    if (zone_name == 'ALL'):
        ...
        datas = Tracksheet.objects.filter(lane_name__in=(zone_name.split(',')),
        ...
        if user is not None:
            if user.is_active:
                ...
                messages.info(request,
                              _(u"loggedin sucessfully. Please check navigation bar on top to fill required forms."))
...
def HomePage(request):
    # print(main)
    analytics = initialize_analyticsreporting()
    print("analyticsanalytics", analytics)
    response = get_report(analytics)
    print("111111111", response)
    recd_response = print_response(response)
    print('recd respoanse', recd_response)
    # print(dimension)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if Tracksheet.objects.filter(date=date, lane_name=laneName).exists():
                ...
                instance.rejected = ((instance.drywaste_bf + instance.wetwaste_bf) - (
                            instance.drywaste_af + instance.wetwaste_af))
...
def MapPage(request):
    ...
    return render(request, "map_fromFGIS.html", context)
...
def RatingView(request):
    ...
    if request.method == 'POST':
        ...
        sub = Rating(name=name, mobile=mobile, email=email, service_swk=service_swk, timing_swk=timing_swk,
                     mobile_swk=mobile_swk, compost_kit_garden=compost_kit_garden, communicate_swk=communicate_swk,
                     solid_waste_man=solid_waste_man, service_workers=service_workers, segregation=segregation,
                     recycle_process=recycle_process, awareness=awareness, role=role)
        # if sub.save():
        # print(sub.save)
        sub.save()
...
def Grievance(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if (send_mail('Grievance received for swk.communitygis.net', message_mail, from_email, to_emails,
                          fail_silently=False, )):
